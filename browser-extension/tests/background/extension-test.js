...
describe('Extension', () => {
  ...
  describe('#firstRun', () => {
    ...
    it('does not open a new tab in local/no-auth builds', async () => {
      fakeSettings.noAuth = true;
      fakeSettings.localApi = true;
      await ext.firstRun({});
      assert.notCalled(fakeChromeAPI.tabs.create);
      assert.notCalled(fakeTabState.activateTab);
    });
  ...
  describe('#init', () => {
    ...
    describe('when a tab is updated', () => {
      ...
      it('does not update the badge count in local/no-auth builds', () => {
        fakeSettings.noAuth = true;
        const tab = createTab();
