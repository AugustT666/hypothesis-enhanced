...
export type GuestToSidebarCalls = CommonCalls & {
  ...
  /**
   * Request a screenshot of the visible tab (as a data URL) from the
   * sidebar, which has access to the extension's `chrome.tabs` API.
   *
   * `callback` receives the data URL or `null` if screenshots are not
   * available (eg. the client is not running inside the extension).
   */
  captureVisibleTab(callback: (dataURL: string | null) => void): void;
...
export type HostToGuestCalls = CommonCalls & {
   ...
   *
   * Rectangle annotations are free-form document regions. They preserve the
   * drawn rectangle, overlapping elements and source text for later recovery.
   */
