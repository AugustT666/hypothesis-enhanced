 ...
 */
/**
 * Compact description of an element which overlaps a rectangle annotation.
 *
 * Fork extension: document rectangles record every element which intersects
 * the drawn shape, so consumers without vision (eg. AI agents) can locate the
 * annotated region in the page structure even when the page later changes.
 */
export type ElementExcerpt = {
  ...
  /** The element's `class` attribute (whitespace-normalized and truncated). */
  class?: string;

  /** Simple XPath from the document body to the element. */
  xpath?: string;

  /**
   * Absolute source URL for image-like elements (`<img>`, `<video>`,
   * `<object>`), or a CSS `background-image` URL. This is the primary
   * locator for annotating a figure or paper image.
   */
  src?: string;

  /** Short text / accessible-name excerpt for the element. */
  text?: string;
};
...
export type ShapeSelector = {
   ...
   * - "element" - The element identified by the annotation's
   *   {@link ElementSelector}. Coordinates are fractions of the element's
   *   bounding box (0-1, origin at the top-left of the rendered element).
   */
  anchor?: 'page' | 'element';
  ...
  /**
   * Fork extension: a short excerpt of the visible text contained in a
   * document rectangle at the time it was created, in DOM order.
   *
   * This is a locator hint for consumers (eg. AI agents) to find the same
   * text on the live page, not a complete context dump.
   */
  regionText?: string;

  /**
   * Fork extension: elements which intersected a document rectangle when it
   * was created. Used to re-locate the annotated region in the page and to
   * give non-visual consumers element-level context.
   */
  elements?: ElementExcerpt[];

  /**
   * Fork extension: the annotated rectangle in viewport (client) coordinates.
   * Recorded when the shape is not anchored to an element (a "document"
   * rectangle), so that the exact drawn region is preserved.
   */
  clientRect?: {
    left: number;
    top: number;
  ...
  /**
   * Fork extension: the viewport size at the time {@link clientRect} was
   * recorded.
   */
  viewport?: {
    width: number;
...
/**
 * Selector which identifies an element in an HTML document.
 *
 * This selector is used in combination with a {@link ShapeSelector} with
 * `anchor: "element"` to identify which element a shape annotation refers to.
...
export type ElementSelector = {
  ...
  /**
   * Lower-case tag name of the element. Used for diagnostics and to decide
   * how the annotation is displayed (eg. whether a thumbnail can be
   * rendered).
   */
  tagName?: string;

  /**
   * The element's `id` attribute, if present. A compact hint which lets
   * consumers (eg. AI agents) locate the element quickly.
   */
  id?: string;

  /**
   * The element's `class` attribute (whitespace-normalized and truncated),
   * if present. Another locator hint for consumers.
   */
  class?: string;

  /**
   * Absolute URL of the element's image source, for `<img>` elements (with
   * `srcset`, the currently displayed variant). Used as a fallback when
   * re-anchoring if the XPath is stale.
   */
  src?: string;

  /**
   * Simple XPath (see `xpathFromNode` in the annotator) from the document
   * body to the element. Used to locate the element when re-anchoring.
   */
  xpath?: string;

  /**
   * Fork extension: an excerpt of the element's outer HTML (truncated).
   * This is the primary context given to consumers (eg. AI agents): the
   * markup of the selected block, which is enough to identify it on the
   * page.
   */
  html?: string;

  /**
   * Fork extension: whether the element's visual content can be rendered to
   * a canvas thumbnail. False for cross-origin images without CORS headers,
   * which taint canvases and cannot be captured.
   */
  renderable?: boolean;
};
...
export type APIAnnotationData = {
  ...
  /**
   * Fork extension: extra data attached to the annotation, such as a
   * screenshot of the annotated region (`extra.screenshot`, a JPEG data URL).
   * Consumed by the sidebar and included in JSON exports for agent
   * consumption.
   */
