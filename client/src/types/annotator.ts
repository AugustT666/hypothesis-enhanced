...
export type AnnotationData = ClientAnnotationData &
  Pick<APIAnnotationData, 'target' | 'uri' | 'extra'> & {
...
export type IntegrationBase = {
   ...
   *
   * @param element - For integrations which support shape annotations on
   *   arbitrary elements, the element which the user selected as the
   *   annotation target (see {@link pickImageTarget}).
   */
  describe(
    root: HTMLElement,
    region: Range | Shape,
  ...
  /**
   * Let the user select the element they want to annotate (eg. an image, a
   * paragraph, a card...).
   *
   * Implementations should highlight the element under the pointer and
   * resolve with the element, plus the position of the click that selected
   * it, when the user clicks. Resolve with `null` if the user cancels the
   * selection.
   *
   * This is used for the "point" shape annotation tool in integrations where
   * the annotation target cannot be reliably determined from the click
   * alone, such as elements in ordinary web pages.
   */
  pickImageTarget?(): Promise<{
    element: Element;
    x: number;
    y: number;
  } | null>;
  ...
  /**
   * Remove any UI associated with {@link pickImageTarget} (eg. an outline
   * highlighting the selected element). Called when annotation creation
