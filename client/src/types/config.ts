...
export type ConfigFromHost = {
   ...
   *
   * When true, the client does not prompt the user to log in and sends API
   * requests with a fixed local token. Intended for use with a local or
   * self-hosted annotation service that does not require authentication.
   */
  noAuth?: boolean;

  /**
   * Whether API requests should be served from browser `localStorage` instead
   * of a backend annotation service.
   *
   * This enables the client to run entirely offline with no login: annotation
   * data is stored in the client's own origin (in the extension, the
   * extension's origin, shared by all tabs).
   */
  localApi?: boolean;
