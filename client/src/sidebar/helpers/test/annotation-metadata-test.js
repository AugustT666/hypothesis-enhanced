...
describe('sidebar/helpers/annotation-metadata.hasRenderableImageElement', () => {
  const fakeAnnotation = (props = {}) => ({
    document: {},
    uri: 'http://example.com/a/page',
    target: [{}],
  ...
  it('returns true for document rectangles with a stored screenshot', () => {
    const annotation = fakeAnnotation();
    annotation.extra = { screenshot: 'data:image/jpeg;base64,xxx' };
    ...
    assert.isTrue(hasRenderableImageElement(annotation));
  });

  it('returns false for document rectangles without a screenshot', () => {
    const annotation = fakeAnnotation();
    annotation.target[0].selector = [
      {
        type: 'ShapeSelector',
    ...
    assert.isFalse(hasRenderableImageElement(annotation));
  });

  it('returns true for PDF shapes without an element selector', () => {
    const annotation = fakeAnnotation();
    annotation.target[0].selector = [
...
describe('sidebar/helpers/annotation-metadata.shapeExcerpt', () => {
  const fakeAnnotation = (props = {}) => ({
    document: {},
    uri: 'http://example.com/a/page',
    target: [{}],
  ...
  it('prefers the element HTML excerpt', () => {
    const annotation = fakeAnnotation({
      target: [
        {
          selector: [
    ...
    assert.equal(shapeExcerpt(annotation), '<img src="x.png" alt="示意图">');
  });

  it('falls back to the shape text when there is no HTML excerpt', () => {
    const annotation = fakeAnnotation({
      target: [
        {
          selector: [
    ...
    assert.equal(shapeExcerpt(annotation), '立即下载');
  });
});
