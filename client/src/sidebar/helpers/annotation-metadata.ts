...
/**
 * Return true if the annotation's shape target is an image-like element
 * (`<img>`, `<picture>`, `<svg>`, `<canvas>`, `<video>`), for which a
 * thumbnail can be rendered. Other element types are displayed using their
 * text content instead.
 */
export function hasRenderableImageElement(
  annotation: APIAnnotationData,
): boolean {
  ...
  const elementSelector = selectors.find(s => s.type === 'ElementSelector') as
    | ElementSelector
    | undefined;
  // Annotations with a stored screenshot always render it as the thumbnail.
  if (annotation.extra?.screenshot) {
    return true;
  }
  if (elementSelector) {
    const tagName = elementSelector.tagName ?? '';
    return (
      elementSelector.renderable !== false &&
      ['img', 'picture', 'svg', 'canvas', 'video'].includes(tagName)
    );
  }
  // Shape annotations anchored to a page (eg. PDFs) can render thumbnails
  // via their integration.
  return selectors.some(s => s.type === 'PageSelector');
}

/**
 * Return the text shown in place of a thumbnail for shape annotations whose
 * element cannot be rendered (eg. cross-origin images): the element's HTML
 * excerpt, or its text content.
 */
export function shapeExcerpt(
  annotation: APIAnnotationData,
): string | undefined {
  const elementSelector = annotation.target[0]?.selector?.find(
    s => s.type === 'ElementSelector',
  ) as ElementSelector | undefined;
  if (elementSelector?.html) {
    return elementSelector.html;
  }
  return shape(annotation)?.text;
}
