...
import { localLinks, localRoutes } from './local-api';
...
export class APIRoutesService {
  ...
  constructor(settings: SidebarSettings) {
    ...
    this._localApi = !!settings.localApi;
  ...
  routes(): Promise<RouteMap> {
    if (!this._routeCache) {
      this._routeCache = this._localApi
        ? Promise.resolve(localRoutes())
        : getJSON<IndexResponse>(this._apiURL).then(result => result.links);
    }
  ...
  links(): Promise<LinksResponse> {
    if (!this._linkCache) {
      this._linkCache = this._localApi
        ? Promise.resolve(localLinks())
        : this.routes().then(async routes => {
