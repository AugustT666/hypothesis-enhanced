...
export class AuthService extends EventEmitter<Events> {
  ...
  async getAccessToken(): Promise<string | null> {
    // In "no auth" mode (used with a local/self-hosted annotation service),
    // there is no login flow. A fixed token is sent; the service is expected
    // to ignore it.
    if (this._settings.noAuth) {
      return 'local';
    }
  ...
  async login({ action = 'login' }: LoginOptions = {}) {
    // In "no auth" mode the user is always considered logged in.
    if (this._settings.noAuth) {
      return;
    }
  ...
  async logout() {
    if (this._settings.noAuth) {
      // There is nothing to revoke in "no auth" mode.
      this._tokenInfoPromise = null;
