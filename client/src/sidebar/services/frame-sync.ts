...
export class FrameSyncService {
  ...
  private _connectGuest(port: MessagePort, sourceId: string | null) {
    ...
    // Capture a screenshot of the visible tab. This only works when the
    // client runs inside the browser extension, where the sidebar app has
    // access to the `chrome.tabs` API.
    guestRPC.on(
      'captureVisibleTab',
