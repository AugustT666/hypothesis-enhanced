...
import { localAPICall } from './local-api';
...
type APIMethodCallbacks = {
  ...
  /** Whether API calls are served from browser local storage. */
  localApi: boolean;
};
...
function createAPICall(
  ...
  return async (params, data, signal) => {
    ...
    try {
      if (localApi) {
        // Serve the API call from browser local storage.
        return await localAPICall(
          route,
          params,
