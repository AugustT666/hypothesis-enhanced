...
function UserMenu({ frameSync, onLogout, settings }: UserMenuProps) {
  ...
  // In "no auth" (local) mode there is no user profile page; the display
  // name is a plain label rather than a link.
  const noAuth = !!settings.noAuth;

  const onProfileSelected = () =>
    isThirdParty && frameSync.notifyHost('profileRequested');
  const profileHref =
    isSelectableProfile && !isThirdParty && !noAuth
      ? store.getLink('user', { user: username })
      : undefined;

  // In "no auth" mode (local annotation service) there is no account to
  // manage or log out of.
  const menuLabel = (
    <span className="p-1">
      <ProfileIcon />
    </span>
