...
describe('Annotation', () => {
  ...
  describe('annotation thumbnail', () => {
    ...
    it('renders a thumbnail if annotation has a shape selector', () => {
      ...
      annotation.target[0].selector = [
        // A page selector marks this as a page-anchored shape (eg. a PDF),
        // which can be rendered as a thumbnail.
  ...
  it('renders overlapping elements recorded by a document rectangle', () => {
    const annotation = fixtures.defaultAnnotation();
    annotation.target[0].selector = [
      {
        type: 'ShapeSelector',
    ...
    const wrapper = createComponent({ annotation });
    const details = wrapper.find('[data-testid="rectangle-elements"]');

    assert.isTrue(details.exists());
    assert.include(details.text(), '矩形覆盖的网页元素');
    assert.include(details.text(), '#figure-1');
  });
