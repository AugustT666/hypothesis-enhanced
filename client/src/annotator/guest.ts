...
import { elementFromPoint } from './anchoring/html-element';
...
import { cropScreenshotToDataURL } from './util/capture-screenshot';
  ...
  implements Annotator, Destroyable
{
   ...
   */
  private _toolActivatedByKeyboard = false;
  ...
  async createAnnotation(tool: AnnotationTool): Promise<AnnotationData | null> {
    if (tool === 'selection') {
      ...
      let restarted = false;
      let picked: { element: Element; x: number; y: number } | null = null;
      try {
        ...
        if (
          tool === 'point' &&
          this._integration.pickImageTarget &&
          ...
          const blockNextClick = (event: Event) => {
          ...
          document.addEventListener('click', blockNextClick, { capture: true });
        ...
        if (picked && tool === 'point') {
          shape = { type: 'point', x: picked.x, y: picked.y };
        } else {
        ...
        const target: Target[] = [
          {
            source: info.uri,
            selector: await this._integration.describe(
              this.element,
              shape,
              describeElement,
            ),
