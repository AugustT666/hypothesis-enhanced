...
describe('Guest', () => {
  ...
  beforeEach(() => {
    ...
    FakePortRPC = sinon.stub().callsFake(() => {
      const rpc = {
        call: sinon.stub().callsFake((method, ...args) => {
          // Answer screenshot requests with `null` by default.
  ...
  describe('events from sidebar frame', () => {
    ...
    describe('on "createAnnotation" event', () => {
      ...
      describe('with element selection (`pickImageTarget`)', () => {
        let pickedElement;

        beforeEach(() => {
          pickedElement = document.createElement('p');
          document.body.append(pickedElement);
          pickedElement.getBoundingClientRect = () =>
            new DOMRect(10, 20, 300, 100);

          fakeIntegration.pickImageTarget = sinon.stub();
          fakeIntegration.clearImageTarget = sinon.stub();
        });

        afterEach(() => {
          delete fakeIntegration.pickImageTarget;
          delete fakeIntegration.clearImageTarget;
          pickedElement.remove();
        });

        it('uses the element selection click as the point for "point" tool', async () => {
          const guest = createGuest();

          fakeIntegration.pickImageTarget.resolves({
          ...
          assert.calledOnce(fakeIntegration.clearImageTarget);
        });

        it('draws a free document rectangle without selecting an element', async () => {
          const guest = createGuest();
          ...
          // Rectangles deliberately use page-level coordinates plus the
          // intersecting element excerpts. This is less fragile than forcing
          // the user to choose one anchor element before drawing.
          assert.notCalled(fakeIntegration.pickImageTarget);
          // Sidebar activation defaults to move mode; the rectangle remains
          // free-form because no element is selected.
        ...
        it('attaches a cropped screenshot to rect annotations when available', async () => {
          createGuest();
          const PNG_1PX =
            'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNkYPhfDwAChwGA60e6kgAAAABJRU5ErkJggg==';

          sidebarRPC()
            .call.withArgs('captureVisibleTab', sinon.match.func)
            .callsFake((method, callback) => callback(PNG_1PX));
          ...
          assert.ok(annotation.extra?.screenshot);
          assert.match(annotation.extra.screenshot, /^data:image\/jpeg/);
        });

        it('blocks the click which follows an element selection', async () => {
          createGuest();
          ...
          await emitHostEvent('createAnnotation', { tool: 'point' });

          // The click belonging to the selection gesture (targeted at the
          // picked element) should be blocked so that picking a link doesn't
          // navigate the page.
          const clickEvent = new MouseEvent('click', {
            bubbles: true,
            cancelable: true,
          });
          pickedElement.dispatchEvent(clickEvent);
          ...
          pickedElement.dispatchEvent(auxClickEvent);
          assert.isTrue(auxClickEvent.defaultPrevented);

          // Clicks on unrelated elements (eg. the toolbar) pass through.
          ...
          toolbarButton.dispatchEvent(toolbarClick);
          assert.isFalse(toolbarClick.defaultPrevented);
          toolbarButton.remove();
        });

        it('does not capture a screenshot for pins on image elements', async () => {
          createGuest();
          ...
          // Pins show the element's HTML/text excerpt in the sidebar instead
          // of a screenshot.
          assert.isUndefined(annotation.extra?.screenshot);
          assert.isFalse(
            sidebarRPC().call.calledWith('captureVisibleTab', sinon.match.func),
          );
          img.remove();
        });

        it('does not capture a screenshot for pins on text elements', async () => {
          createGuest();
          fakeIntegration.pickImageTarget.resolves({
            element: pickedElement,
            x: 150,
        ...
        it('cancels annotation creation if element selection is canceled', async () => {
          createGuest();

          fakeIntegration.pickImageTarget.resolves(null);
