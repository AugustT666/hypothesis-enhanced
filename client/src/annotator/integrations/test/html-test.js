...
describe('HTMLIntegration', () => {
  ...
  let fakeElementAnchoring;
  let fakeElementPicker;
  ...
  beforeEach(() => {
    ...
    fakeElementAnchoring = {
      anchorShape: sinon.stub(),
      describeDocumentRect: sinon.stub(),
      describeShape: sinon.stub(),
      locateElement: sinon.stub(),
    };
    ...
    $imports.$mock({
      ...
      '../anchoring/html-element': {
        ...fakeElementAnchoring,
        ElementPicker: sinon.stub().returns(fakeElementPicker),
      },
  ...
  it('implements `anchor` using element anchoring for shape selectors', async () => {
    const integration = createIntegration();
    const selectors = [{ type: 'ShapeSelector', shape: { type: 'point' } }];

    await integration.anchor({}, selectors);
    assert.calledWith(fakeElementAnchoring.anchorShape, selectors);
    assert.notCalled(fakeHTMLAnchoring.anchor);
  });
  ...
  describe('#describe', () => {
    ...
    it('describes shapes using element anchoring', () => {
      const integration = createIntegration();
      const root = {};

      const shape = { type: 'point', x: 0, y: 0 };
      const element = document.createElement('p');
      integration.describe(root, shape, element);
      assert.calledWith(fakeElementAnchoring.describeShape, shape, element);
      assert.notCalled(fakeHTMLAnchoring.describe);
    });

    it('describes shapes without a selected element as document rects', () => {
      const integration = createIntegration();
      const shape = { type: 'rect', left: 0, top: 0, right: 10, bottom: 10 };
      integration.describe({}, shape);
      assert.calledWith(fakeElementAnchoring.describeDocumentRect, shape);
      assert.notCalled(fakeElementAnchoring.describeShape);
    });
  });

  describe('#pickImageTarget', () => {
    it('returns the result of picking an element', async () => {
      const integration = createIntegration();
      const element = document.createElement('p');
      fakeElementPicker.pick.resolves({ element, x: 10, y: 20 });

      const result = await integration.pickImageTarget();
      assert.deepEqual(result, { element, x: 10, y: 20 });
    });

    it('removes the picker UI when cleared', async () => {
      const integration = createIntegration();
      fakeElementPicker.pick.resolves(null);

      await integration.pickImageTarget();
      integration.clearImageTarget();
      assert.calledOnce(fakeElementPicker.destroy);
    });
  });

  describe('#supportedTools', () => {
    it('supports text, rect and point tools by default', () => {
      const integration = createIntegration();
      assert.deepEqual(integration.supportedTools(), [
        'selection',
        'rect',
        'point',
      ]);
    ...
    it('does not support image tools if `html_image_annotation` flag is false', () => {
      features.update({ html_image_annotation: false });
      const integration = createIntegration();
      assert.deepEqual(integration.supportedTools(), ['selection']);
    });
  });
...
describe('HTMLIntegration#renderToBitmap (document rect screenshot)', () => {
  // 1x1 transparent PNG.
  const PNG_1PX =
    'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNkYPhfDwAChwGA60e6kgAAAABJRU5ErkJggg==';

  it('renders the stored screenshot for document rectangles', async () => {
    const integration = new HTMLIntegration({ features: new FeatureFlags() });

    const anchor = {
      annotation: {
        $tag: 't:1',
        extra: { screenshot: PNG_1PX },
      },
      target: {
        selector: [
          {
    ...
    const bitmap = await integration.renderToBitmap(anchor, { maxWidth: 100 });
    assert.ok(bitmap);
    assert.isAtLeast(bitmap.width, 1);
    assert.isAtLeast(bitmap.height, 1);
    bitmap.close();
  });

  it('throws for document rectangles without a screenshot', async () => {
    const integration = new HTMLIntegration({ features: new FeatureFlags() });
    ...
    let error;
    try {
      await integration.renderToBitmap(anchor, { maxWidth: 100 });
    } catch (e) {
      error = e;
    }
    assert.ok(error);
