...
describe('Toolbar', () => {
  ...
  it('uses the same free-form rectangle flow when the button is Shift-clicked', () => {
    const createAnnotation = sinon.stub();
    const wrapper = createToolbar({
      createAnnotation,
      supportedTools: ['selection', 'rect'],
    ...
    const button = wrapper.find('button[data-testid="rect-annotation"]');
    button.simulate('click', { shiftKey: true });

    assert.calledWith(createAnnotation, 'rect');
    assert.equal(createAnnotation.firstCall.args.length, 1);
  });
  ...
  describe('rectangle annotation button title', () => {
    it('describes the free-form rectangle flow when keyboard mode is not active', () => {
      const wrapper = createToolbar({
        supportedTools: ['selection', 'rect'],
        activeTool: 'rect',
        keyboardActive: false,
