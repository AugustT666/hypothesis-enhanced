...
const DATA_DIR =
  ...
  (process.platform === 'win32'
    ? join(process.env.LOCALAPPDATA ?? process.env.APPDATA ?? '', 'H Local')
...
/**
 * Best-effort removal of the state file. Never throws: a stale state file is
 * harmless because `isRunning` reports false for a dead pid, and environments
 * that shim `fs.rmSync` (eg. trash-redirecting sandboxes) may fail the delete.
 */
function clearState() {
  try {
    rmSync(STATE_FILE, { force: true });
  } catch {
    // Ignore: stale state is cleaned up on the next start.
  }
}
