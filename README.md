...
- **保留官方功能**：网页文字划选批注、高亮、标签、回复、搜索、PDF 批注等官方能力仍然保留。
- **官方服务一键切换**：在扩展设置页即可选择「官方服务」，使用官方 Hypothes.is 账号登录并在线同步，无需重新构建（默认仍是本地模式）。
...
默认构建为免登录本地模式。需要使用官方账号、与官方 `hypothes.is` 服务在线同步批注时：

1. 打开扩展设置页（扩展管理页 → 「扩展选项」，或插件内头像菜单）；
2. 在「账号与数据源」中选择 **官方服务（Hypothes.is 账号）**；
3. 刷新已打开的页面（或新开页面），在侧边栏顶部点击「登录 / 注册」使用官方账号登录。

切换后：批注读写、标签、群组、搜索、个人资料均走官方服务；批注计数徽章也在此模式下生效。若同时加入了局域网房间，房间优先；在设置页清除房间地址后回到官方服务。

也可不经过运行时切换、直接构建官方在线版扩展：`make build SETTINGS_FILE=settings/chrome-prod.json`（见 `browser-extension/settings/`）。
