#!/usr/bin/env bash
# 一键构建 H Local：先构建 client，再构建并打包浏览器扩展。
set -euo pipefail
...
echo "==> 安装扩展依赖"
(cd browser-extension && $EXT_YARN install --mode=skip-build 2>/dev/null || $EXT_YARN install)

echo "==> 构建扩展 (settings/chrome-local.json)"
(cd browser-extension && make build SETTINGS_FILE=settings/chrome-local.json)

echo "==> 打包"
(cd browser-extension && make dist/h-local.zip SETTINGS_FILE=settings/chrome-local.json)

echo ""
echo "完成！产物: browser-extension/dist/h-local.zip"
